...
def major_vote(starts, ends, ents):
    ...
    """
    if not starts or not ends or not ents:
        return []
    zipped_starts = list(zip(*starts))
    zipped_ends = list(zip(*ends))
    zipped_ents = list(zip(*ents))
    
    min_len = min(len(zipped_starts), len(zipped_ends), len(zipped_ents))
    
    new_starts = [0]*min_len
    new_ends = [0]*min_len
    new_ents = [0]*min_len

    for i in range(min_len):
        list_start = list(zipped_starts[i])
        new_starts[i] = max(set(list_start), key=list_start.count)

        list_end = list(zipped_ends[i])
        new_ends[i] = max(set(list_end), key=list_end.count)

        list_ent = list(zipped_ents[i])
